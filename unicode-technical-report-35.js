...
function format(date, formatString) {
  ...
  const replaceToken = (token, value) => {
    formatString = formatString.replace(new RegExp(token, 'g'), value);
  };
  ...
  replaceToken('MM', (d.month() + 1).toString().padStart(2, "0"));
  replaceToken('M', d.month() + 1);
  replaceToken('MMMM', lM[d.month()]);
  replaceToken('MMM', sM[d.month()].slice(0, 3));
